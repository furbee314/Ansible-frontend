...
Invoked from the module frontend via cockpit.spawn(); subcommands:
  ...
  deploy     --hosts IP[,IP] --user USER [--port N] --target PLAYBOOK
             ...
             auto-wrapped in a writable per-job temp dir). All ansible output
             streams to stdout. Exits with ansible's exit code.
...
def cmd_deploy(args):
    ...
    if info["needs_wrap"]:
        ...
        wrapper = os.path.join(inv_dir, "__deployer_job_%d.yml" % os.getpid())
        ...
        run_target = wrapper
        print("[deployer] task file detected — wrapped as %s (includes %s)\n"
